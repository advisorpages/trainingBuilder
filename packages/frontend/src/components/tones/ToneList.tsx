...
export const ToneList: React.FC<ToneListProps> = ({ onEdit, onDelete }) => {
  ...
  const fetchTones = async (params?: ToneQueryParams) => {
    try {
      setLoading(true);
      setError(null);

      const queryParams: ToneQueryParams = {
        page: currentPage,
        limit: 20,
        ...params,
      };
      ...
      const response = await toneService.getTones(queryParams);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-gray-600">
        Showing {tones.length} of {total} tones
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {tones.map((tone) => (
              <tr key={tone.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm font-medium text-gray-900">{tone.name}</div>
                </td>
                <td className="px-6 py-4">
                  <div className="text-sm text-gray-500">
                    {tone.description || 'No description provided'}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span
        ...
        {tones.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            No tones found. {searchTerm && 'Try adjusting your search criteria.'}
          </div>
        )}
      </div>
