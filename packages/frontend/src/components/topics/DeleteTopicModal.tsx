      ...
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        ...
        <div className="text-center">
          <h3 className="text-lg font-medium text-gray-900 mb-2">
            {topic.isActive ? 'Deactivate Topic' : 'Delete Topic'}
          </h3>

          <p className="text-sm text-gray-500 mb-4">
            {topic.isActive ? (
              <>
                Are you sure you want to deactivate "<strong>{topic.name}</strong>"?
                This will make it unavailable for new sessions, but existing sessions will remain unaffected.
              </>
            ) : (
              <>
                Are you sure you want to permanently delete "<strong>{topic.name}</strong>"?
          ...
          >
            {isDeleting ? (
              <span className="flex items-center">
                ...
                {topic.isActive ? 'Deactivating...' : 'Deleting...'}
              </span>
            ) : (
