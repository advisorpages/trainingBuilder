...
export class AddVariantSelectionToAIInteractionType1738850000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    // Add 'variant_selection' to ai_interaction_type_enum
    ...
    await queryRunner.query(`
      ...
      BEGIN
        IF NOT EXISTS (
          SELECT 1
          FROM pg_enum
          WHERE enumlabel = 'variant_selection'
          AND enumtypid = (
            SELECT oid
            FROM pg_type
          ...
          ALTER TYPE ai_interaction_type_enum ADD VALUE 'variant_selection';
        END IF;
      END
      $$;
    `);
