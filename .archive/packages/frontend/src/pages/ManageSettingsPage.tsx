...
import { DeleteSettingModal } from '../components/settings/DeleteSettingModal';
...
export const ManageSettingsPage: React.FC = () => {
  ...
  const canManageSettings = user?.role?.name === 'Content Developer' || user?.role?.name === 'Broker';
  ...
  const handleCreateSetting = async (data: CreateSettingRequest) => {
    try {
      ...
      showNotification('error', error.response?.data?.message || 'Failed to create setting');
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleUpdateSetting = async (data: UpdateSettingRequest) => {
    if (!selectedSetting) return;

    try {
      setIsSubmitting(true);
      await settingsService.updateSetting(selectedSetting.key, data);
      showNotification('success', 'Setting updated successfully');
      setViewMode('list');
      setSelectedSetting(null);
      setRefreshTrigger(prev => prev + 1);
      fetchCategories(); // Refresh categories in case category was changed
    } catch (error: any) {
      showNotification('error', error.response?.data?.message || 'Failed to update setting');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDeleteSetting = async () => {
    ...
    try {
      setIsDeleting(true);
      await settingsService.deleteSetting(settingToDelete.key);
      showNotification('success', 'Setting deleted successfully');
      setSettingToDelete(null);
      setRefreshTrigger(prev => prev + 1);
      fetchCategories(); // Refresh categories
    } catch (error: any) {
      showNotification('error', error.response?.data?.message || 'Failed to delete setting');
    } finally {
      setIsDeleting(false);
    }
  };

  const handleResetSetting = async (setting: SystemSetting) => {
    try {
      await settingsService.resetToDefault(setting.key);
      showNotification('success', 'Setting reset to default value');
      setRefreshTrigger(prev => prev + 1);
    } catch (error: any) {
      showNotification('error', error.response?.data?.message || 'Failed to reset setting');
    }
  };
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                ...
                <p className="mt-1 text-sm text-yellow-700">
                  You need Content Developer or Broker permissions to manage system settings.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Manage Settings</h1>
