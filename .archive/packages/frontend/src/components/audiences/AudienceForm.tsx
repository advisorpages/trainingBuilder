...
export const AudienceForm: React.FC<AudienceFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData: CreateAudienceRequest | UpdateAudienceRequest = {
        name: formData.name.trim(),
        description: formData.description.trim() || undefined,
      };

      if (audience) {
        (submitData as UpdateAudienceRequest).isActive = formData.isActive;
      }
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
      <h3 className="text-lg font-medium text-gray-900 mb-4">
        {audience ? 'Edit Audience' : 'Create New Audience'}
      </h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {audience && (
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Inactive audiences won't be available for new sessions
            </p>
          </div>
        )}
