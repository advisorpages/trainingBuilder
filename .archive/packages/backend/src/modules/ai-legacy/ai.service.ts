...
P.S. ${urgency === 'high' ? 'This truly is your final chance - act now!' : 'Remember, this exclusive opportunity won\'t last forever.'}`;
  ...
  private adjustSectionDurations(sections: FlexibleSessionSection[], targetTotal: number): void {
    ...
    const remainingAdjustment = adjustment - (adjustmentPerSection * adjustableSections.length);
  ...
  private generateTopicTitle(category: string, specificTopics = '', relevantTopics: Topic[], topicNumber: number): string {
...
- Drive registrations and engagement`;
  ...
  private generateLearningOutcomes(category: string, desiredOutcome: string, isExercise = false): string {
